...
function cash_out_now(element, section_no, increment = '') {
   ...
   // game_id = current_game_data.id
    ...
    var cashOutData = {
        UserId: 1,
        GameId:1,
        cashOutX: incrementor,
        cashOutAmount: bet_amount * incrementor
    };



    $.ajax({
        url: 'http://localhost:5172/api/aviator/insertCashout',
        type: "POST",
        ...
        success: function (result) {
            ...
            if (result.isSuccess) {
                ...
                if (section_no == 0) {
                    ...
                    let is_main_auto_bet_checked = false; // $("#main_auto_bet").prop('checked');
...
function new_game_generated() {
  ...
  //hiding bets  $("#all_bets .mCSB_container").html('');
...
function update_bet_list(bets, target, appendType = '') {
    ...
    for (i = 0; i < bets.length; i++) {
       // console.log(bets[i])
        ...
        html += '<div class="list-items ' + isActive + ' ' + sectionNo + ' ' + '">' +
                    '<div class="column-1 users"> <img src="./assets/images/av-' + bets[i].userid + '.png" class="avatar me-1"> ' + bets[i].userid + ' </div>' +
    ...
    $(target).append(html);
   // console.log(bets, target, appendType)
...
function place_bet_now() {
    // let  totalBalance = Number($("#wallet_balance").text().replace(/[^0-9]/g, ''));
    // let totalBet = bet_array.reduce((accumulator, currentObject) => {
    //     return accumulator + Number(currentObject.bet_amount);
    // }, 0);
    // let currentBalanceNumber = totalBalance - totalBet;
    // let currentBalance = `$${currentBalanceNumber.toLocaleString()}`

    // console.log($("#wallet_balance").text().replace(/,/g, ''))

    // $("#wallet_balance").text(currentBalance);
    // $("#header_wallet_balance").text(currentBalance);
    ...
    $.ajax({
        url: 'http://localhost:5172/api/aviator/insertBet',
        type: "POST",
        contentType: 'application/json',
        data: JSON.stringify(insertBetData),
        success: function (result) {
            updateBalance();
        },
