...
function setVariable(is_plan = '') {
    ...
    if (canvasWidth < 992) {
        ...
        imgyposition = 45;
        imgxposition = 10;
        imgTag.src = "./assets/images/unnamed.png";
        settimeinterval = 40;
...
/* without requestanimationFrame and browser throttling, no tab changing bug but less smooth animation 
...
function animatePathDrawing(ctx, x0, y0, x1, y1, x2, y2, duration, imgTag) {
    ...
    const step = function() {
        ...
        if (!start) {
            start = now;
            expectedEndTime = start + duration;
        }
        ...
        if (progress < 1) {
            // Calculate the ideal timeout delay, adjusting for any execution delays.
            const idealDelay = duration * progress;
            const actualDelay = now - start;
            const nextDelay = Math.max(16, idealDelay - actualDelay);

            setTimeout(step, nextDelay);
        }
    };

    setTimeout(step, 0); // Start the animation
}
...
function draw(spritesheet, x, y, width, height, timePerFrame, numberOfFrames, ctx, frameIndex) {
    // console.log(spritesheet, (frameIndex * width / numberOfFrames), 0, (width / numberOfFrames), height, x, y, (width / numberOfFrames), height)
